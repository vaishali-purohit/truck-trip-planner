...
class TestTripApi(APITestCase):
    def test_trip_plan_creates_trip_and_returns_payload(self):
        fake_plan = {
            ...
            "totalDistanceMi": 100.0,
            "drivingHours": 2.0,
            "totalTripTimeHours": 4.0,
            "compliance": "compliant",
            "driverLogs": "completed",
            "estimatedArrivalISO": "2026-01-01T12:00:00+00:00",
            "stopsCount": 0,
            "routeInstructions": [],
            "eldLogSheets": [],
        ...
        with patch("trips.views.build_trip_plan", return_value=fake_plan):
        ...
        self.assertEqual(t.pickup_location, "Chicago, IL")

    def test_trip_plan_validates_cycle_hours_used(self):
        with patch("trips.views.build_trip_plan") as mocked:
            res = self.client.post(
                reverse("trip-plan"),
                {
                    "currentLocation": "Chicago, IL",
    ...
    def test_trip_plan_value_error_returns_400(self):
        with patch("trips.views.build_trip_plan", side_effect=ValueError("bad input")):
            res = self.client.post(
                reverse("trip-plan"),
                {
                    "currentLocation": "X",
    ...
    def test_trip_plan_upstream_error_returns_502(self):
        with patch("trips.views.build_trip_plan", side_effect=requests.RequestException("down")):
            res = self.client.post(
                reverse("trip-plan"),
                {
                    "currentLocation": "Chicago, IL",
    ...
    def test_trip_list_and_detail(self):
        t1 = TripPlan.objects.create(
            ...
            dropoff_location="C",
            cycle_hours_used=1,
            result={"pickup": {"city": "A", "state": "AA"}, "dropoff": {"city": "B", "state": "BB"}},
        )
        t2 = TripPlan.objects.create(
            ...
            dropoff_location="C2",
            cycle_hours_used=2,
            result={"pickup": {"city": "X", "state": "XX"}, "dropoff": {"city": "Y", "state": "YY"}},
        )
        ...
        self.assertIn("tripNo", res.data[0])
    ...
    def test_locations_search_upstream_failure_returns_502(self):
        with patch("trips.views.requests.get", side_effect=requests.RequestException("down")):
